...
public class ContractStatusCellFactory implements Callback<ListView<ContractStatus>, ListCell<ContractStatus>>
{
	@Override
	public ListCell<ContractStatus> call(ListView<ContractStatus> param)
	{
		final ListCell<ContractStatus> cell = new ListCell<ContractStatus>(){
			@Override
			protected void updateItem(ContractStatus item, boolean empty)
			{
				super.updateItem(item, empty);
				if(item == null || empty)
					this.setText(null);
				...
				{
					this.setText(item.getContrStatusCode());
				}
			}
		};
